...
SparkProtocol::SparkProtocol()
    : QUEUE_SIZE(sizeof(queue)), handlers({sizeof(handlers), NULL}), expecting_ping_ack(false),
      initialized(false), updating(false), product_id(PRODUCT_ID),
      product_firmware_version(PRODUCT_FIRMWARE_VERSION)
{
    queue_init();
}

void SparkProtocol::init(const char* id, const SparkKeys& keys, const SparkCallbacks& callbacks,
...
CoAPMessageType::Enum SparkProtocol::received_message(unsigned char* buf, size_t length)
{
...
void SparkProtocol::hello(unsigned char* buf, bool newly_upgraded)
{
    ...
    size_t len = Messages::hello(buf + 2, message_id, newly_upgraded, PLATFORM_ID, product_id,
                                 product_firmware_version, false, nullptr, 0);
    wrap(buf, len);
}
...
void SparkProtocol::function_return(unsigned char* buf, unsigned char token, int return_value)
{
    unsigned short message_id = next_message_id();
...
bool SparkProtocol::send_event(const char* event_name, const char* data, int ttl,
                               EventType::Enum event_type)
{
    ...
    {
        // // EDIT: increasing rate limit here to test on local cloud
        //
...
bool SparkProtocol::send_subscription(const char* event_name, SubscriptionScope::Enum scope)
{
    uint16_t msg_id = next_message_id();
                                         ...
                                         const char* id)
{
    ...
    for (int i = 0; i < NUM_HANDLERS; i++)
    {
        if (event_handlers[i].handler == handler &&
            event_handlers[i].handler_data == handler_data && event_handlers[i].scope == scope)
        {
...
bool SparkProtocol::handle_chunk(msg& message)
{
    ...
    *(msg_to_send + 1) = 16;
    ...
    if (0xFF == queue[payload])
    {
        ...
        file.chunk_size = message.len - payload -
        ...
        DEBUG("chunk idx=%d crc=%d fast=%d updating=%d", chunk_index, crc_valid, fast_ota,
              updating);
        if (crc_valid)
        {
...
bool SparkProtocol::handle_update_done(msg& message)
{
    ...
    *(msg_to_send + 1) = 16;
    ...
    coded_ack(msg_to_send + 2, message.token,
              missing ? ChunkReceivedCode::BAD : ChunkReceivedCode::OK, queue[2], queue[3]);
...
void SparkProtocol::handle_event(msg& message)
{
    ...
    for (int i = 0; i < NUM_HANDLERS; i++)
    {
        ...
        if (0 == cmp)
        {
            ...
            {
                descriptor.call_event_handler(sizeof(FilteringEventHandler), &event_handlers[i],
...
CoAPMessageType::Enum SparkProtocol::handle_received_message(void)
{
    ...
    if (len > QUEUE_SIZE)
    { // TODO add sanity check on data, e.g. CRC
    ...
    return handle_message(message, token, message_type) ? message_type : CoAPMessageType::ERROR;
}
...
void SparkProtocol::separate_response(unsigned char* buf, unsigned char token, unsigned char code)
{
    separate_response_with_payload(buf, token, code, NULL, 0);
}

void SparkProtocol::separate_response_with_payload(unsigned char* buf, unsigned char token,
...
int SparkProtocol::set_key(const unsigned char* signed_encrypted_credentials)
{
    ...
    if (0 != decipher_aes_credentials(core_private_key, signed_encrypted_credentials, credentials))
        return DECRYPTION_ERROR;

    calculate_ciphertext_hmac(signed_encrypted_credentials, credentials, hmac);

    if (0 == verify_signature(signed_encrypted_credentials + 128, server_public_key, hmac))
    {
...
inline void SparkProtocol::coded_ack(unsigned char* buf, unsigned char token, unsigned char code,
                                     unsigned char message_id_msb, unsigned char message_id_lsb)
{
    buf[0] = 0x61; // acknowledgment, one-byte token
    buf[1] = code;
