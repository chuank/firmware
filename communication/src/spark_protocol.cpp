 ...
 * @param seed  A random value from a cryptographically secure random number
 * generator.
 */
inline void default_random_seed_from_cloud(unsigned int seed) { srand(seed); }
bool SparkProtocol::is_initialized(void) { return initialized; }
...
SparkProtocol::SparkProtocol()
    : QUEUE_SIZE(sizeof(queue)), handlers({sizeof(handlers), NULL}),
      expecting_ping_ack(false), initialized(false), updating(false),
      product_id(PRODUCT_ID),
      product_firmware_version(PRODUCT_FIRMWARE_VERSION) {
  queue_init();
}

void SparkProtocol::init(const char *id, const SparkKeys &keys,
...
int SparkProtocol::handshake(void) {
  ...
  err = set_key(queue);
  if (err) {
    ERROR("Handshake:  could not set key, %d");
    return err;
  }
  ...
  if (!event_loop(CoAPMessageType::HELLO,
                  2000)) // read the hello message from the server
  {
...
bool SparkProtocol::event_loop(CoAPMessageType::Enum &message_type) {
  ...
  if (2 <= bytes_received) {
    ...
    if (updating) {
      system_tick_t millis_since_last_chunk =
          callbacks.millis() - last_chunk_millis;
      ...
      system_tick_t millis_since_last_message =
          callbacks.millis() - last_message_millis;
...
int SparkProtocol::blocking_send(const unsigned char *buf, int length) {
  ...
  while (length > byte_count) {
    bytes_or_error =
        callbacks.send(buf + byte_count, length - byte_count, nullptr);
    if (0 > bytes_or_error) {
      // error, disconnected
...
int SparkProtocol::blocking_receive(unsigned char *buf, int length) {
  ...
  while (length > byte_count) {
    bytes_or_error =
        callbacks.receive(buf + byte_count, length - byte_count, nullptr);
    if (0 > bytes_or_error) {
      // error, disconnected
...
CoAPMessageType::Enum SparkProtocol::received_message(unsigned char *buf,
                                                      size_t length) {
...
void SparkProtocol::hello(unsigned char *buf, bool newly_upgraded) {
  ...
  size_t len =
      Messages::hello(buf + 2, message_id, newly_upgraded, PLATFORM_ID,
                      product_id, product_firmware_version, false, nullptr, 0);
  wrap(buf, len);
}
...
void SparkProtocol::variable_value(unsigned char *buf, unsigned char token,
...
int SparkProtocol::variable_value(unsigned char *buf, unsigned char token,
                                  unsigned char message_id_msb,
                                  unsigned char message_id_lsb,
...
bool SparkProtocol::send_event(const char *event_name, const char *data,
  ...
  if (is_system_event) {
    ...
    //
    // system_tick_t now = recent_event_ticks[evt_tick_idx] =
    // callbacks.millis();
    // evt_tick_idx++;
    // evt_tick_idx %= 11;
  ...
  size_t msglen = Messages::event(queue + 2, msg_id, event_name, data, ttl,
                                  event_type, false);
...
void SparkProtocol::send_subscriptions() {
  const int NUM_HANDLERS =
      sizeof(event_handlers) / sizeof(FilteringEventHandler);
  for (int i = 0; i < NUM_HANDLERS; i++) {
    if (NULL != event_handlers[i].handler) {
      if (event_handlers[i].device_id[0]) {
        send_subscription(event_handlers[i].filter,
                          event_handlers[i].device_id);
      } else {
...
bool SparkProtocol::event_handler_exists(const char *event_name,
...
bool SparkProtocol::add_event_handler(const char *event_name,
                                      EventHandler handler, void *handler_data,
                                      SubscriptionScope::Enum scope,
                                      const char *id) {
  ...
  for (int i = 0; i < NUM_HANDLERS; i++) {
    if (NULL == event_handlers[i].handler) {
      ...
      memset(event_handlers[i].filter + FILTER_LEN, 0,
...
void SparkProtocol::chunk_received(unsigned char *buf, unsigned char token,
...
int SparkProtocol::send_missing_chunks(int count) {
  ...
  while ((idx = next_chunk_missing(chunk_index_t(idx))) != NO_CHUNKS_MISSING &&
         sent < count) {
    buf[(sent * 2) + 7] = idx >> 8;
...
void SparkProtocol::chunk_missed(unsigned char *buf,
...
int SparkProtocol::description(unsigned char *buf, unsigned char token,
                               unsigned char message_id_msb,
                               unsigned char message_id_lsb, int desc_flags) {
  buf[0] = 0x61; // acknowledgment, one-byte token
  buf[1] = 0x45; // response code 2.05 CONTENT
  buf[2] = message_id_msb;
  buf[3] = message_id_lsb;
...
ProtocolState::Enum SparkProtocol::state() { return ProtocolState::READ_NONCE; }
...
bool SparkProtocol::handle_update_begin(msg &message) {
  ...
  *(msg_to_send + 1) = 16;
  ...
  coded_ack(msg_to_send + 2, success ? 0x00 : RESPONSE_CODE(4, 00), queue[2],
            queue[3]);
  ...
  if (success) {
    if (!callbacks.prepare_for_firmware_update(file, 0, NULL)) {
      DEBUG("starting file length %d chunks %d chunk_size %d", file.file_length,
            file.chunk_count(file.chunk_size), file.chunk_size);
      last_chunk_millis = callbacks.millis();
      chunk_index = 0;
      chunk_size = file.chunk_size; // save chunk size since the descriptor size
                                    // is overwritten
      this->updating = 1;
      // when not in fast OTA mode, the chunk missing buffer is set to 1 since
      // the protocol
      // handles missing chunks one by one. Also we don't know the actual size
      // of the file to
...
bool SparkProtocol::handle_chunk(msg &message) {
  ...
  *(msg_to_send + 1) = 16;
  ...
  while (queue[payload] != 0xFF) {
    ...
    option++;
    payload +=
        (queue[payload] & 0xF) + 1; // increase by the size. todo handle > 11
  }
  if (0xFF == queue[payload]) {
    ...
    file.chunk_size =
        message.len - payload -
        queue[message.len - 1]; // remove length added due to pkcs #7 padding?
    ...
    DEBUG("chunk idx=%d crc=%d fast=%d updating=%d", chunk_index, crc_valid,
          fast_ota, updating);
    if (crc_valid) {
...
bool SparkProtocol::handle_update_done(msg &message) {
  ...
  *(msg_to_send + 1) = 16;
  ...
  coded_ack(msg_to_send + 2, message.token,
            missing ? ChunkReceivedCode::BAD : ChunkReceivedCode::OK, queue[2],
            queue[3]);
...
bool SparkProtocol::function_result(const void *result, SparkReturnType::Enum,
                                    uint8_t token) {
  // send return value
  queue[0] = 0;
  queue[1] = 16;
...
bool SparkProtocol::handle_function_call(msg &message) {
  ...
  msg_to_send[0] = 0;
  msg_to_send[1] = 16;
  coded_ack(msg_to_send + 2, has_function ? 0x00 : RESPONSE_CODE(4, 00),
            queue[2], queue[3]);
  if (0 > blocking_send(msg_to_send, 18)) {
    // error
    return false;
  ...
  auto callback = [=](const void *result, SparkReturnType::Enum resultType) {
    return this->function_result(result, resultType, message.token);
  };
...
void SparkProtocol::handle_event(msg &message) {
  ...
  while (next_src < end && 0x00 == (*next_src & 0xf0)) {
    ...
    *next_dst++ = '/';
    if (next_dst != next_src) {
      // at least one extra byte has been used to encode a CoAP Uri-Path option
      // length
  ...
  for (int i = 0; i < NUM_HANDLERS; i++) {
    ...
    const size_t filter_length =
        strnlen(event_handlers[i].filter, MAX_FILTER_LENGTH);
    ...
    if (0 == cmp) {
      ...
      if (!this->descriptor.call_event_handler) {
        if (event_handlers[i].handler_data) {
          ...
          handler(event_handlers[i].handler_data, (char *)event_name,
                  (char *)data);
        } else {
        ...
        descriptor.call_event_handler(
            sizeof(FilteringEventHandler), &event_handlers[i],
            (const char *)event_name, (const char *)data, NULL);
      }
    }
...
bool SparkProtocol::send_description(int description_flags, msg &message) {
  int desc_len = description(queue + 2, message.token, queue[2], queue[3],
                             description_flags);
...
bool SparkProtocol::handle_message(msg &message, token_t token,
                                   CoAPMessageType::Enum message_type) {
  switch (message_type) {
  case CoAPMessageType::DESCRIBE: {
    if (!send_description(DESCRIBE_SYSTEM, message) ||
        !send_description(DESCRIBE_APPLICATION, message)) {
      return false;
    }
    break;
  }
  ...
  case CoAPMessageType::VARIABLE_REQUEST: {
    ...
    if (SparkReturnType::BOOLEAN == var_type) {
      ...
      int buf_size = variable_value(queue + 2, token, queue[2], queue[3],
                                    str_val, str_length);
  ...
  case CoAPMessageType::TIME:
    handle_time_response(queue[6] << 24 | queue[7] << 16 | queue[8] << 8 |
                         queue[9]);
    break;
...
CoAPMessageType::Enum SparkProtocol::handle_received_message(void) {
  ...
  return handle_message(message, token, message_type) ? message_type
...
void SparkProtocol::handle_time_response(uint32_t time) {
  // deduct latency
  uint32_t latency =
      last_chunk_millis ? (callbacks.millis() - last_chunk_millis) / 2000 : 0;
  last_chunk_millis = 0;
  callbacks.set_time(time - latency, 0, NULL);
}

unsigned short SparkProtocol::next_message_id() { return ++_message_id; }

unsigned char SparkProtocol::next_token() { return ++_token; }
...
void SparkProtocol::separate_response(unsigned char *buf, unsigned char token,
                                      unsigned char code) {
...
inline void SparkProtocol::coded_ack(unsigned char *buf, unsigned char code,
                                     unsigned char message_id_msb,
                                     unsigned char message_id_lsb) {
...
inline void SparkProtocol::coded_ack(unsigned char *buf, unsigned char token,
                                     unsigned char code,
                                     unsigned char message_id_msb,
                                     unsigned char message_id_lsb) {
  buf[0] = 0x61; // acknowledgment, one-byte token
